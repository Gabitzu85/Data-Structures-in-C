...
{
	int val;    //Valoare retinuta de catre nod.
  struct stack_node_t *next; //Pointer catre urmatorul element de tip nod.
}stack_node_t;
...
{
	struct stack_node_t *head;  //Capul listei inlantuite.
  unsigned int length;    //Dimensiunea stivei.
}stack_t;
...
{
  while(stack_empty(stack)!=0)	//afiseaza val scoasa = 3 10 7
	{							// daca stack_empty(stack) == 0 afiseaza doar 3 10 7
